...
def clean_up(txt, term_doc, nlp):
    if re.search("([^\x00-\x7F])+", txt):
        ...
        txt_doc = nlp(txt)
        sim_score = 0
        if(txt_doc and txt_doc.vector_norm):
            sim_score = txt_doc.similarity(term_doc)
        if sim_score == 0 or sim_score == 1.0 or sim_score < 0.9:
            return ''
        txt_list = []
        for token in txt_doc:
            if token.text not in nlp.vocab:
                ...
                if '\'' not in token.text:
                    txt_list.append(token.text)

    return ' '.join([x for x in txt_list if x.isalpha()])

def get_semantically_similar_terms(term, nlp):
    results = get_json(REST_URL + "/search?q=" + term.replace(' ', '+') + "&require_exact_match=true&also_search_properties=true")["collection"]
    syn_list = []
    term_doc = nlp(term)
    for res in results:
        if "synonym" in res:
            ...
            if type(syn) is list:
                for syn_word in syn:
                    if syn_word != term:
                        syn_list.append(clean_up(syn_word, term_doc, nlp))
            else:
                if syn != term:
                    syn_list.append(clean_up(syn, term_doc, nlp))
